import { ChangeDetectionStrategy, Component, computed, contentChild, InjectionToken, input, ViewEncapsulation } from '@angular/core';
import { NgTemplateOutlet } from '@angular/common';
import { RouterLink } from '@angular/router';
import { Button, ButtonModule, ButtonPassThrough, ButtonStyle } from "primeng/button";
...
const CUSTOM_VARIANTS_PROPS = {
  ...
  [CUSTOM_VARIANTS['SECONDARY']]: {
    variant: 'outlined',
    size: 'small',
    styleClass: 'font-primary text-primary-700 border-primary-700 bg-primary-100/40 h-9 rounded-lg border-1 px-2 py-0 text-lg font-normal hover:cursor-pointer md:h-10 md:text-2xl',
  }
}
/** 
...
export class CustomButton extends Button {
  ...
  variantCustom = input<'primary' | 'secondary' | 'accent'>();
  isUnstyled = computed(() => this.unstyled() || [CUSTOM_VARIANTS['PRIMARY']].some((el: string) => this.variantCustom() === el));
  clabel = computed(() => this.innerContent() ? '' : this.label);
  cbadge = computed(() => this.innerContent() ? '' : this.badge);
  cpt = computed<ButtonPassThrough>(() => CUSTOM_VARIANTS_PROPS[this.variantCustom() || '']?.pt || this.pt());
  cptLabel = computed(() => ({ root: (this.cpt() as unknown as { label?: string })?.label || '' }));
  cvariant = computed(() => CUSTOM_VARIANTS_PROPS[this.variantCustom() || '']?.variant as ('outlined' | 'text' | undefined) || this.variant);
  csize = computed(() => CUSTOM_VARIANTS_PROPS[this.variantCustom() || '']?.size as ('small' | 'large' | undefined) || this.size);
  cstyleClass = computed(() => `${CUSTOM_VARIANTS_PROPS[this.variantCustom() || '']?.styleClass || ''} ${this.styleClass || ''}`);
  target = input<string>();
  rel = input<string>();
