...
const storeStructureExample: StoreState<any, any> = {
  queries: {
    'query-1': {
      // key is always query id, or search params (new URLSearchParams({ name: 'John', age: '30' }).toString()), values are always arrays of entity ids
  ...
  entities: {
    'entity-1': {
      // key is always entity id, value is the entity data
...
export abstract class BasicApiService<T, K> {
  abstract fetchOne(
    id: string,
    options?: { isURL?: boolean | null },
  ): Observable<K>;
  abstract fetchMany(
    params: object,
  ): Observable<{ data: T[]; page: number; pageSize: number; total: number }>;
  abstract updateOne(id: string, data: Partial<K>): Observable<K>;
  abstract deleteOne(id: string): Observable<object | void>;
}

export abstract class BasicStore<T, K> {
  protected abstract _apiService: BasicApiService<T, K>;
  ...
  protected _removeEntity(id: string) {
    ...
    queryRemovalMap.forEach(([key]) => {
      restQueries[key] = {
        ...restQueries[key],
        ids: restQueries[key].ids.filter((qid) => qid !== id),
      };
    });
    this._updateState({
  ...
  protected _restoreIdInQueries(
    id: string,
    queryRemovalMap: [string, number][],
  ) {
  ...
  fetchEntityById(
    ...
    return this._apiService.fetchOne(id, { isURL }).pipe(
      tap((entity: K) => {
        if (!entity) {
          this._setEntityDetails(id, null, false, 'Entity not found');
          return;
        }
  ...
  // the approach of selecting data via function get* could be mistake, because every time I call the function it will create new observable
  // Usually I use async pipe in template, but will it unsubscribe from previous observable and subscribe to new one every time, I don't know
  /**
  ...
  getEntityByURL(url: string): Observable<ActionStatus<K>> {
    ...
    return merge(entity$, loadEntity$).pipe();
  }
  ...
  updateEntityPessimistic(
    ...
    fn: (
      id: string,
      data?: any,
    ) => Observable<K> = this._apiService.updateOne.bind(this._apiService),
  ) {
  ...
  removeEntityPessimistic(
    id: string,
    fn: (id: string) => Observable<any> = this._apiService.deleteOne.bind(
      this._apiService,
    ),
  ) {
    this.setIsLoading(id, true);
  ...
  clearEntityInStore(id: string) {
    this._removeEntity(id);
  }

  removeEntityOptimistic(
    id: string,
    fn: (id: string) => Observable<any> = this._apiService.deleteOne.bind(
      this._apiService,
    ),
  ...
  fetchEntities(
    params: object = {
      page: this.defaultStoreState.page,
      pageSize: this.defaultStoreState.pageSize,
    },
  ) {
    const queryKey = toQueryKey(params);
    ...
    return this._apiService.fetchMany(params).pipe(
      ...
      tap((response: any) => {
        ...
        const newEntities = response.data.reduce(
          (acc: Record<string, EntityState<T, K>>, entity: T) => {
            acc[(entity as any)?.[this.primaryIdKey]] = {
        ...
        this._updateState({
          queries: {
            ...this._state.getValue().queries,
            [queryKey]: {
              ids: response.data.map((e: any) => e[this.primaryIdKey]),
              isLoading: false,
              error: null,
            },
          },
  ...
  getEntities(
    ...
    const entities$ = this.state$.pipe(
      map((state) => {
        ...
        const entities = ids
          ? ids
              .map((id) => state.entities[id]?.preview)
              .filter((p): p is T => !!p)
          : undefined;
        return {
          data: entities,
