...
export interface AIHouse {
  ...
  rating: number;
  metadata?: any;
  likes?: string[]; // implement likes table for that later
  url: string;
  seo: SEO
  createdAt: Date;
  updatedAt: Date;
...
export type AIHouseBasic = Pick<AIHouse, 'id' | 'name' | 'description' | 'address' | 'price' | 'rooms' | 'area' | 'images' | 'rating' | 'available' | 'url'>;

export type AIHouseCreate = Omit<AIHouse, 'id' | 'createdAt' | 'updatedAt' | 'rating' | 'likes'> & { ownerId: string };
